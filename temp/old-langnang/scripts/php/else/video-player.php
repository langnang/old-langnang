<?php
// 音频文件类型
$video_type_list = ["mp4", "avi", "flv", "rmvb", "mkv"];
$dir_list = [];
$file_path = "";
$file_list = [];
  ...
  <div class="container" style="margin-top: 20px;">
    ...
    <div class="list-group">
      ...
      if (!is_dir(__DIR__ . $path)) {
        echo '<video id="video" controls src="' . $path . '" autoplay style="width: 100%;  overflow: hidden" ended="next"></video>';
      }
      ...
      foreach ($file_list as $value) {
        if (in_array(strtolower(explode(".", $value)[count(explode(".", $value)) - 1]), $video_type_list)) {
          ...
          echo '<a href="' . implode("/", array_slice($path_array, 0, count($path_array) - ($is_dir ? 0 : 1))) . "/" . $value . '" class="list-group-item ' . $is_active . '"><span class="glyphicon glyphicon-facetime-video" aria-hidden="true"></span>&nbsp;&nbsp;' . $value . "</a>";
        }
      }
      ?>
    </div>
